...
// Files required by the first rendered pool. Warming the HTTP cache while the
// starter screen is visible removes most network stalls after Start Design is
// clicked without lowering texture resolution or changing final materials.
const INITIAL_SCENE_ASSETS = [
  ...ENVIRONMENT_ASSETS,
...
function preloadEnvironmentAssets() {
  ...
  window.__poolEnvironmentPreloadPromise = Promise.allSettled(
    INITIAL_SCENE_ASSETS.map((url) => fetch(url, {
      cache: "force-cache",
      priority: ENVIRONMENT_ASSETS.includes(url) ? "high" : "auto",
      credentials: "omit"
    }))
  );
...
function scheduleEditorPreload() {
  ...
  // Start on the next task rather than waiting up to 900 ms for idle time.
  // The starter page is static, so module parsing can happen before the user
  // selects a pool and does not reduce final visual quality.
